...
public class JwtAuthenticationConverter implements Converter<@NonNull Jwt, @NonNull AbstractAuthenticationToken> {
    ...
    public AbstractAuthenticationToken convert(@NonNull Jwt jwt) {
        ...
        if (principalName == null) {
            log.warn("El claim principal ('{}') no se encontro en el token JWT. Usando 'sub' como fallback.",
                    principleAttribute);
            principalName = jwt.getClaimAsString(JwtClaimNames.SUB);
            if (principalName == null) {
                log.error("Ni el claim principal ('{}') ni 'sub' se encontraron en el JWT. Usando ID como fallback.",
                        principleAttribute);
    ...
    private Collection<? extends GrantedAuthority> extractResourceRoles(Jwt jwt) {
        Map<String, Object> realmAccess = Optional.ofNullable(jwt.getClaimAsMap(REALM_ACCESS_CLAIM))
                .orElseGet(() -> {
                    log.debug("El claim '{}' no esta presente en el JWT.", REALM_ACCESS_CLAIM);
        ...
        Collection<String> realmRoles = Optional.ofNullable((Collection<String>) realmAccess.get(ROLES_CLAIM))
                .orElseGet(() -> {
                    log.debug("El claim '{}' no esta presente. No se extraeran roles.", ROLES_CLAIM);
    ...
    private String getPrincipleClaimName(Jwt jwt) {
        return Optional.ofNullable(principleAttribute)
                ...
                .orElseGet(() -> {
                    log.debug("El principle-attribute '{}' no esta presente. Usando 'sub'.", principleAttribute);
